...
for( my $i = 0; $i < @trainingImages; $i++ )
  ...
  for( my $j = 0; $j < @trainingImages; $j++ )
    ...
    my $commandFile = "${outputPrefix}AffineCommand.sh";
 ...
 #     print FILE "@regArgs\n\n";
      ...
      my @xfrmArgs = ( "${ANTsPath}/antsApplyTransforms",
                     ...
                     '-i', $trainingLabels[$j],
                     '-r', $trainingLabels[$i],
                     '-o', "${outputPrefix}AffineLabelsWarped.nii.gz",
                     '-n', 'NearestNeighbor',
    ...
    if( ! -e "${outputPrefix}AffineLabelsWarped.nii.gz" )
      {
      print "** registration ${outputPrefix}\n";
      $count++;
      if( $which == 0 )
        {
        system( "qsub -N ${count}_ANTS -q standard -l nodes=1:ppn=1 -l walltime=05:00:00 -l mem=2gb $commandFile" );
        }
      else
        {
        system( "qsub -N ${count}_ants -q nopreempt -l nodes=1:ppn=1 -l walltime=05:00:00 -l mem=2gb $commandFile" );
        }
      }
    else
      {
