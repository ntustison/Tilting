...
Arno Klein\,$^{4}$, Satrajit Ghosh\,$^{5}$, Luis Ibanez\,$^{6}$, and Brian A. Avants\,$^{7}$
 }
...
\footnote{
http://www.nitrc.org
}.  While we welcome and support all efforts to evaluate such
...
incentivized to spend months or years fine tuning their methods' parameters
and invest relatively little effort exploring the parameter space of external
methods.
...
computational methods, specifically in, but not
...
bias} \citep{sackett1979} (considering software  as a type of
...
Theorems'' provides bounds on solution quality that show otherwise
i.e. it specifically demonstrates that ``improvement of performance 
in problem-solving
...
the author must consider if it is possible to incorporate similar prior
knowledge into existing methods.
...
a recent paper \citep{haegelen2013} (similar issues can
...
Given the authors extensive experience and investment with ANIMAL, 
the reader can assume some confidence in minimal instrumentation 
bias although inclusion of the specific ANIMAL parameters used
would be informative and preferred.  For SyN, however, we do not
have equivalent assurance of experience with the algorithm nor
are any parameters listed for independent verification
...
Tools like the nipype \citep{gorgolewski2011} capture
...
to those of us who are, understanding someone else's code, perhaps
implemented in an unfamiliar programming language and poor coding style, 
