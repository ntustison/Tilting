...
provide access to an open source distribution and are indexed in a resource like the Neuro-informatics Tools and Resources Clearinghouse \cite{NITRC_CITE_HERE}}.  However, complementing the
...
the FMRIB Software Library (FSL) \cite{Jenkinson2012}, \added[id=hj]{Slicer, \cite{SlicerRefNeeded}} and
...
component involves the comparison of algorithms\added[id=tr]{, which in priciple is
...
we propose an \replaced[id=hj]{preliminary}{initial} set of guidelines for minimal
...
restrict the search space \added[id=hj]{to the relevant parameters for a narrow specified problem}.  The very idea that one\replaced[id=tr]{(e.g., registration)}{, for instance,
...
If you \replaced[id=hj]{must re-implment}{do}, \replaced[id=hj]{consider making your implementaiton available}{be sure to make your implementation available.}}
