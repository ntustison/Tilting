...
experiment is performed) and in publishing \replaced[id=tr]{its details and results}{the evaluation}.
\replaced[id=tr]{Here, we}{We} suggest general guidelines for both of these steps most of which
...
restrict the search space.  The very idea that one\replaced[id=tr]{(e.g., registration)}{, for instance,
