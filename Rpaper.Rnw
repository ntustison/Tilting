...
by the increased availability of high-performance computational resources \hjEdit{like XCEDE\ref{xcede_web_ref};}\hjDelete{,} large-scale
communal \hjEdit{development} efforts such as the Insight Toolkit, and the R statistical project\hjDelete{,}\hjEdit{;} and
...
\end{abstract}
\begin{keyword}
open science \sep \hjEdit{open data} \sep \hjEdit{comparison} \sep reproducibility  \sep \trEdit{software} \sep \trEdit{instrument bias}
...
of imaging\trEdit{-related} software \hjEdit{analysis packages}.
Established packages such as SPM \cite{Ashburner2012},
the FMRIB Software Library (FSL) \cite{Jenkinson2012}, \hjEdit{Slicer, \cite{SlicerRefNeeded}} and
...
As fellow scientists who actively participate in \hjDelete{this}\hjEdit{neuroscience} 
