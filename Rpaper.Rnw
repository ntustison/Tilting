...
\title{\replaced[id=ak]{Instrumentation bias in the use and evaluation of [scientific/brain imaging] software\\or\\Building on a stronger foundation: the case for open and transparent software development and evaluation practices in the neurosciences\\or\\How to carefully test and evaluate brain imaging software
}{\newline Tilting at Algorithmic Windmills}
}
...
\begin{abstract}
Exploration of neuroscience hypotheses \added[id=tr]{has} been  enhanced
by the increased availability of high-performance computational resources
\replaced[id=hj,remark={XSEDE is the new teragrid https://www.xsede.org/ }]{like XSEDE\ref{xcede_web_ref};}{,} large-scale
\replaced[id=nt]{developmental}{communal \added[id=hj]{development}} efforts such as the Insight Toolkit and the R statistical project\replaced[id=hj]{;}{,} and
algorithmic advancements for data transformations and processing.
An integral component of \replaced[id=hj]{a rigorous}{the} vetting process for
novel algorithmic techniques includes comparison with other methods previously
established within the community.  Public availability, \deleted[id=hj]{including
open source distribution,} of established packages has significantly
facilitated these comparative evaluations\added[id=hj]{, particularly when the packages
provide access to an open source distribution and are indexed in a resource like the \replaced[id=nt]{Neuroimaging Informatics Tools and Resources Clearinghouse}{Neuro-informatics Tools and Resources Clearinghouse}}%
\added[id=nt]{\footnote{
http://www.nitrc.org
...
issues associated with \deleted[id=tr]{a type of} measurement bias in comparative algorithmic
evaluations\replaced[id=hj]{.}{and } \added[id=hj]{In this paper we} propose a set of guidelines for authors and reviewers to minimize \replaced[id=hj]{these evaluation biases}{this confound}.
\replaced[id=hj]{Finally we}{We also} illustrate these guidelines
through the use of a recent real-world scenario.
\end{abstract}
\begin{keyword}
open science \sep \added[id=hj]{open data} \sep \added[id=hj]{comparison} \sep reproducibility  \sep \added[id=tr]{software} \sep \added[id=tr]{instrument bias}
...
who is intelligent to make up their mind.}{\deleted[id=tr]{Professor }Richard Feynman}
\added[id=tr,remark={I think the Feynman quote would be much more appropriately positioned
...
of imaging\added[id=tr]{-related} \replaced[id=hj]{analysis software packages}{software}.
Established packages such as SPM \cite{Ashburner2012},
the FMRIB Software Library (FSL) \cite{Jenkinson2012}, \added[id=hj]{Slicer, \cite{fedorov2012}} and
...
\replaced[id=hj,remark={I did not know the original intent of this sentence, so I made a new one}]{In conjunction with distributing robust software tools, neuroimaging packages also continue to incorporate}{Parallel to this line of inquiry is continued}
algorithmic innovation for improvement
in analysis tools.

As \deleted[id=hj,remark={I don't understand the next couple of sentences -- It is a bit too abstract.}]{fellow} scientists who actively participate in \replaced[id=hj]{neuroscience}{this} \added[id=hj]{}
research through our many contributions to the Insight
Toolkit
\added[id=nt]{\cite{johnson2007,tustison2009a,tustison2012a}} 
and other packages such as the Advanced Normalization Tools (ANTs),%
\footnote{
http://www.picsl.upenn.edu/ANTs
}
we have noticed \replaced[id=tr]{an increasing number of}{several} publications in which a \replaced[id=tr]{principal}{principle}
component involves the comparison of algorithms\added[id=tr]{, which in principle is
a good thing}.  \replaced[id=tr]{We are concerned, however, by}{One of our concerns is} the lack of detail with which these comparisons are
...
Based on our experience (including mistakes that we \added[id=tr]{ourselves} have made),
we propose a \replaced[id=hj]{preliminary}{initial} set of guidelines for minimal
reporting of algorithmic usage to minimize such bias, understanding that
the discussion will \replaced[id=hj]{likely}{hopefully}
elicit a much more comprehensive response from the larger \added[id=hj]{neurosciences} community.

\replaced[id=tr]{Previous}{Although} articles have discussed similar concerns, oftentimes
within a \replaced[id=tr]{specific application}{much larger} context (e.g. fMRI reporting \cite{poldrack2008}),
we are \replaced[id=tr]{more generally}{particularly} interested in comparative evaluations of software
...
It is hoped that this commentary serves to raise awareness \replaced[id=tr]{of}{to} both
...
\section{Guidelines}
\added[id=ak,remark={
there are noteworthy breaches for each of the subsubsection heading guidelines.  including these would help broaden the scope of the argument beyond the current focus.  i am concerned that readers will get the impression that you are trying to vindicate yourselves after being slighted by another's unfair study.}]{}

\replaced[id=hj]{A comparative analysis paper's longevity and impact on future scientific
...
impact}.  \replaced[id=hj]{A complete evaluation requires preparatory effort}{This requires effort in both planning the evaluation}(before any
experiment is performed) and \replaced[id=hj]{effort to publish}{in publishing} \replaced[id=tr]{its details and results}{the evaluation}.
\replaced[id=tr]{Here, we}{We} suggest general guidelines for both of these steps \deleted[id=hj]{most of} which
derive from basic scientific principles of clarity and reproducibility.

\subsection{Designing the Evaluation Study}
\subsubsection{\added[id=hj]{Rigourously }Define the Algorithm's Context}\added[id=sg, remark={Perhaps ``Scope'' is a better word here, as that could cover things beyond the problem to include runtime environments, data characteristics (i.e, quality, scanner, etc.,.)}]
The only algorithm that is capable of outperforming all others on all
problems is exhaustive search\added[id=tr]{, assuming a perfect objective function}.
\replaced[id=tr]{As this is not feasible in practice}{Therefore}, many algorithms employ
prior knowledge such as regularization or other heuristics that
restrict the search space \added[id=hj]{to the relevant parameters for a narrow\added[id=nt]{ly} specified problem}.  The very idea that one\replaced[id=tr]{(e.g., registration)}{, for instance,
registration} algorithm \replaced[id=tr]{could perform}{should be} better than all other algorithms on
all types of data is fundamentally flawed.  Indeed, the \added[id=tr]{``}No Free Lunch
Theorems\replaced[id=tr]{''}{\cite{Wolpert1997}} provide bounds on solution quality that show otherwise
i.e. specifically that ``improvement of performance in problem-solving hinges on using prior information to match procedures to problems.''\added[id=tr]{\cite{Wolpert1997}}

\subsubsection{\replaced[id=hj]{\replaced[id=sg]{Demand that the algorithm developers p}{P}rovide default parameters for the \replaced[id=sg]{context}{algorithm} being investigated}{Hassle the developers for a set of default parameters}}
...
provide optimal parameterization. \added[id=nt]{In comparing FAST (FSL), 
...
Developers should \replaced[id=tr]{actively}{also} facilitate
comparisons by providing default parameter files, sample scripts,
\added[id=sg]{data} or possible command line calls.  For example, the developers of
...
query regarding usage \added[id=sg]{but such unstructured information sources can be difficult to query and navigate}.

\subsubsection{Do not \replaced[id=tr]{implement}{code up} your own version of the algorithm particularly
if one is available from the original authors.
If you \replaced[id=hj]{must re-implement}{do}, \replaced[id=hj]{consider making your implementation available}{be sure to make your implementation available.}}
...
group doing the comparison.  This is not ideal as it \added[id=tr]{is} questionable
...
is due to the algorithm itself\replaced[id=tr]{,}{or} the implementation\added[id=tr]{, or the parameter tuning}\added[id=sg, remark={But this could easily be tested on a validation set - perhaps something to include/talk about}].
...
\added[id=sg, remark={Why not make them provide a reproducible VM for the evaluation? I understand it's a matter of difficulty, but somebody running an evaluation should be able to reproduce it. Furthermore, an evaluation itself is a function of it's context and is obsolete imho after the paper is published unless it can be rerun over and over again with newer datasets and newer versions of the software. Just because ANTS did well in Arno's study, I should not extrapolate that it's still the best! Think of choosing an algorithm much like choosing a model in machine learning.}]

\subsubsection{Include parameters.}
A best practice exemplar of comparative evaluation studies
is the work by Klein et al. \cite{Klein2009} \replaced[id=hj]{where}{in which}
14 nonlinear registration algorithms were compared using
four publicly available\added[id=tr]{,} labeled brain \added[id=tr]{MRI} data sets.
As part of the study, the authors themselves tuned
the parameters for their algorithm to ensure good
performance.  As
part of this work, Prof. Klein \replaced[id=hj]{distributed}{availed} the scripts
...
\added[id=ak,remark = {
can you get the authors of the offending parkinson's study to share their parameter settings or data or to allow you to set their parameters to re-run their analysis?  this would help provide a solid punchline.  why not also demonstrate how to properly compare antsRegistration with another package, such as DARTEL or ANIMAL?
}]{}
  However, for SyN,
...
a wrapper of the \added[id=tr]{Slicer} module in nipype,%
\footnote{
...
differences in \added[id=tr]{FreeSurfer} output that varied with \deleted[id=tr]{Freesurfer} release
...
\added[id=nt]{and/or online public resources should be used}
}
...
\added[id=nt]{Alternatively (or additionally), online resources can
...
\subsection{\replaced[id=nt]{Pertinency of the current publishing paradigm}{Post-Publication: Publications Age}}
\replaced[id=nt]{
...
of the software development process.  We assume that if 
an algorithm is of sufficient utility and/or novelty for publication, 
then it will continue to undergo improvements and bug fixes.  In
...
\added[id=ak, remark={
the ANTs vs. antsRegistration example is pretty involved.  it would make more sense to me if you were to state more clearly that the article is itself an example of what to do (and what not to do) when comparing software implementations of algorithms.  on that note, it would be good to structure this example to correspond closely with the subheading guidelines you offer. 
everything is tuned, somehow.  you've documented what you've done after really everything has been decided.  how did you know how to set the parameters?  why are the ANTs parameters different than in the 2009 evaluation?  how robust are the parameters?  how do we know that after may 3, 2013, everything won't go to hell with a single tweak?
 it would be good to show how and when they fail, not just with settings, but with data of different quality (noise, contrast), or how they perform for different modalities or across modalities.  you've done a lot of training and testing on oasis data over the years.  is it possible that you are tailoring / overfitting?}]{}
...
has been \deleted[id=hj]{more or less} stable for quite some time).
...
\replaced[id=tr]{to facilitate}{facilitating} developer collaboration.  The version
...
Pairwise Students t-tests \added[id=tr]{with Bonferroni correction} were performed for each of the three
comparisons\deleted[id=tr]{ with Bonferroni correction}.
For the affine-only Dice value comparison, \verb#antsRegistration#
only slightly \replaced[id=tr]{outperformed}{output} \verb#ANTS# ($q = 0.053$, mean of the
...
the affine output of the \verb#antsRegistration# runs.%

\added[id=ak,remark={
it seems like cherry picking to choose the better affine result to use for each affine+nonlinear comparison, especially if better is measured by the same criterion for success at the next stage of the evaluation.}]{}
  This
...
improved results over \verb#ANTS#.  Although the \added[id=tr]{abstract algorithms implemented by the different} SyN \added[id=tr]{software} components are theoretically
the same, \added[id=tr]{implementation with} the newer program was constrained to conform
...
from ITK's crowdsourcing of the contributed code (a great example of Linus \added[id=hj]{Torvold's(SP?)} Law
...
\added[id=tr]{(e.g., single vs. double precision, 80-bit x87 vs. 64-bit IEEE~754  computation)} between the
...
\added[id=tr]{[Q: would it be worthwhile showing the variability of results with
  single- vs. double-precision ANTS? Also, does result change with number of
  CPUs? That would be interesting to demonstrate also. Finally, using standard
  vs. SSE double precision?]}
%\added[id=hj]{[Q: As of 2013-05-18 an alpha version of a floating point antsRegistration/ITKv4
%has been completed, and posted on \url{git@github.com:aghayoor/ANTs-1.git on github}.  This version is building, passing %all old
%tests, and producing nearly identical results in single precision for a short run ($10x10x10$ iterations) 3 level %registration]}
