...
communal \added[id=hj]{development} efforts such as the Insight Toolkit, and the R statistical project\replaced[id=hj]{;}{,} and
...
provide access to an open source distribution and are indexed in a resource like the Neuro-informatics Tools and Resources Clearinghouse}%
...
%\added[id=ak]{
%\begin{footnote}
%5. can you get the authors of the offending parkinson's study to share their parameter settings or data or to allow you to set their parameters to re-run their analysis?  this would help provide a solid punchline.
%
%6. why not also demonstrate how to properly compare antsRegistration with another package, such as DARTEL or ANIMAL?
%
%7. there are noteworthy breaches for each of the subsubsection heading guidelines.  including these would help broaden the scope of the argument beyond the current focus.  i am concerned that readers will get the impression that you are trying to vindicate yourselves after being slighted by another's unfair study.
%
%}
...
\replaced[id=hj,remark={I did not know the original intent of this sentence, so I made a new one}]{In conjunction with distributing robust software tools, neuro-imaging packages also continue to incorporate}{Parallel to this line of inquiry is continued}
...
\replaced[id=hj]{A comparative analysis paper's longevity and impact on furture scientific
exploriations is directly related to the completeness of the evaluation}{
A solid evaluation is essential to a paper's longevity and
impact}.  \replaced[id=hj]{A complete evaluation requires prepratory effort}{This requires effort in both planning the evaluation}(before any
...
problems is exhaustive search\added[id=tr]{, assuming a perfect objective function}.\replaced[id=tr]{As this is not feasible in practice}{Therefore}, many algorithms employ
...
\subsubsection{\replaced[id=hj]{Demand that the algorihtm developers provide default parameters for the context being investigated}{Hassle the developers for a set of default parameters}}
...
If you \replaced[id=hj]{must re-implement}{do}, \replaced[id=hj]{consider making your implementaiton available}{be sure to make your implementation available.}}
...
\added[id=ak]{
\begin{itemize}
\item can you get the authors of the offending parkinson's study to share their parameter settings or data or to allow you to set their parameters to re-run their analysis?  this would help provide a solid punchline.
\item why not also demonstrate how to properly compare antsRegistration with another package, such as DARTEL or ANIMAL?
\end{itemize}
}
}
  However, for SyN,
...
\added[id=tr]{[Q: would it be worthwile showing the variability of results with
...
%tests, and producing nearly identical results in single precision for a short run ($10x10x10$ iterations) 3 level %registraiton]}
