...
\added[id=hj,remark={XSEDE is the new teragrid https://www.xsede.org/ }]{like XSEDE\ref{xcede_web_ref};}\deleted[id=hj]{,} large-scale
...
of imaging\added[id=tr]{-related} software \added[id=hj]{analysis packages}.
...
%% I don't unseratnd the next couple of sentences -- It is a bit too abstract.
As fellow scientists who actively participate in \replaced[id=hj]{neuroscience}{this} 
...
\deleted[id=tr]{Although} \added[id=tr]{Previous} articles have discussed similar concerns, oftentimes
within a \deleted[id=tr]{much larger} \added[id=tr]{specific application} context (e.g. fMRI reporting \cite{poldrack2008}), 
we are \deleted[id=tr]{particularly}\added[id=tr]{more generally} interested in comparative evaluations of software
...
It is hoped that this commentary serves to raise awareness \deleted[id=tr]{to}\added[id=tr]{of} both 
...
experiment is performed) and in publishing \deleted[id=tr]{the evaluation}\added[id=tr]{its details and results}.
\deleted[id=tr]{We}\added[id=tr]{Here, we} suggest general guidelines for both of these steps most of which
...
problems is exhaustive search\added[id=tr]{, assuming a perfect objective function}.\deleted[id=tr]{Therefore}\added[id=tr]{As this is not feasible in practice}, many algorithms employ
prior knowledge such as regularization or other heuristics that
restrict the search space.  The very idea that one\deleted[id=tr]{, for instance,
registration}\added[id=tr]{(e.g., registration)} algorithm \deleted[id=tr]{should be}\added[id=tr]{could perform} better than all other algorithms on
all types of data is fundamentally flawed.  Indeed, the \added[id=tr]{``}No Free Lunch
Theorems\added[id=tr]{''} \deleted[id=tr]{\cite{Wolpert1997}} provide bounds on solution quality that show otherwise
...
Developers should \deleted[id=tr]{also}\added[id=tr]{actively} facilitate
...
\subsubsection{Do not \deleted[id=tr]{code up}\added[id=tr]{implement} your own version of the algorithm particularly 
...
is due to the algorithm itself\added[id=tr]{,} \deleted[id=tr]{or} the implementation\added[id=tr]{, or the parameter tuning}.
...
\deleted[id=tr]{facilitating}\added[id=tr]{to facilitate} developer collaboration.  The version
...
only slightly \deleted[id=tr]{output} \added[id=tr]{out}performed \verb#ANTS# ($q = 0.053$, mean of the 
...
\added[id=tr]{[I think the Feynman quote would be much more appropriately positioned
